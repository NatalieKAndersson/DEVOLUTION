...
#' @param eventcutoff The cutoff for two events to be considered the same one.
#' @param datatypes Which datatypes to include in the analysis.
#' @param rule If there is a rule for the allocation
#' @param eps The epsilon
#' @param truncate If the data should be truncated.
#' @param names How the subclones should be named.
#' @param preclustered Whether the matrix has been preclustered or not.
#' @return The event matrix
#' @export
DEVOLUTION <- function(file,eventcutoff,datatypes, rule, eps,truncate,names,preclustered){
  start.time <- Sys.time()
  ...
  file_new <- file
  i <-  1
  versionnames <- c(paste( c("v"), 1:10000, sep=""))
  k <- 1
  ...
  #Changing the version names.
  assign("file_versions", file_new, envir=globalenv())
  i <- 1
  for(i in 1:nrow(file_new)){ #Adding information to the events about which kind of alteration it is.
    
    #Changing the version names as well.
    x <- strsplit(file_new[i,10], "\\s+")
    versions <- length(which(x[[1]]=="v1"))
    if(as.numeric(versions)>0){
      file_new[i,10] <- paste(paste(word(file_new[i,10],1:(as.numeric(length(x[[1]]))-as.numeric(versions))),collapse =" "),paste("v",versions,sep=""),sep=" ")
    }
  ...
  for(i in 2:nrow(overview)){
    ...
    if(i == nrow(overview)){
      if(firststem == 1 && as.numeric(overview[2,2]) == 0){
        ...
        if(preclustered==TRUE){
          f_stem <- matrix(0,1,12)
          f_stem[1,] <- c(unique(file_new[file_new[,1]!="Remove",1]),"ALL",1,1,1,"NA","NA","Stem","WES","Stem","100","ALL")
        }else{
        ...
        if(firststem!=1){
          if(preclustered==TRUE){
            print("New cluster name")
            stemmatrix[,12] <- "ALL" #Giving it a new cluster name.
          }
  ...
  }else if(preclustered == TRUE){
    #The genetic alterations has already been clustered with another method.
    print("Preclustered was set to TRUE. The data has thus already been clustered with another method. We will now use that data instead of dbscan.")
    # The clusters are given in: file_new[,13]
    #Adding them to the overview matrix.
    i <- 1
    overview_new <- cbind(overview,matrix(0,nrow(overview),1)) #Adding the cluster belonging to the overview.
    for(i in 1:nrow(overview_new)){
      
      pos <- which(file_new[,10]==overview[i,1])
      #if(length(unique(file_new[pos,12]))!=1){
      #print("The same alteration are in different clusters across samples.")}
      overview_new[i,ncol(overview_new)] <- file_new[pos[1],12]
      
      i <- i+1
    }
    unique_clusters <- unique(file_new[,12])
    assign("overview_cluster",overview_new,envir=globalenv())
  ...
  for(i in 1:length(unique_clusters)){
    if(unique_clusters[i]== "ALL"){
      ...
      cluster_matrix[i,1] <- paste("Cluster",unique_clusters[i])}
    i <- i+1
  }
  ...
  for(i in 1:as.numeric(nrow(clone_matrix_names))){ #Looping through the rows in the clustermatrix.
    ...
    calculateTC <- matrix(0,1000,1) #Resetting the matrix.
    s <- 1
    t <- t + 1
    ...
    if(as.numeric(clusterTC_order[i,2]) != 0){
      if(i == 1){
        if("ALL" %in% file_new[,2] == TRUE){ #If we have an ALL event we do not want to add the name "Subclone" to it.
          ...
          clusterTC_order[i,3] <- paste("Cluster_",namevector[i])
          clone_matrix_names[i,1] <- paste("Cluster",i)
          clone_matrix_names[i,3] <- paste("Cluster_",namevector[i])
        }
      }else{
        clusterTC_order[i,1] <- paste("Cluster",i)
        clusterTC_order[i,3] <- paste("Cluster_",namevector[i])
  ...
  #################################################################
  #Making a new file indicating which cluster each event belong to#
  #################################################################
  ...
  if(preclustered==TRUE){
    file_new <- file_new[,1:11] #Removing the column with the old clustering. Not needed below.
  }
  ...
  medianmatrix <- matrix(0,max(table(file_samples_subclones[,13]))+2,as.numeric(length(subclones))) #This matrix is to be used in order to calculate the median TC for each subclone within each sample.
  ...
  #Adding the median to the initial file. All events in a cluster in a specific sample will have the median TC.
  #Added 221005
  assign("medianmatrix", medianmatrix, envir=globalenv())
  i <- 1
  for(i in 1:nrow(file_samples_subclones)){
    
    sampleandclone <- file_samples_subclones[i,13]
    pos <- which(medianmatrix[1,]==sampleandclone)
    file_samples_subclones[i,11] <- medianmatrix[nrow(medianmatrix),as.numeric(pos)]
    i <- i+1
  }
  assign("file_samples_subclones_median", file_samples_subclones, envir=globalenv())
          ...
          if(x[2,j] != "0"){ #We do not want to analyze situations where we do not even have the daughter subclone in question.
            ...
            if((as.numeric(justzeros[1])/(as.numeric(nrow(possible_mothers)))) == 1){ #If it is one, then every position is a zero. This means that this is the only solution for this mother-daughter allocation in this sample.
              ...
              if(is.na(match(x[2,1],hundredpercentclones)) == FALSE){ #The subclone is present in a hundredpercentclones.
                ...
                #     if(rule[rule_d,3]=="No"){
                #     print("They are not allowed to be placed after one another.") #Tystade d? jag har en kod h?gre upp som g?r det.
...
#' @param ord How you would like to order the genetic information. If set to TRUE the matrix is ordered by tumor, then by sample and then by chromosome.
