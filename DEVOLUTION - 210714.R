              ...
              if(newname %in% word(theonlymothers[1,],2,3) == TRUE){ #The clone in question has a condition on it.
                ...
                if(maxname %in% space[,1] == TRUE){ #Does the mother exist in the sample in question?
                  ...
                  if(maxname %in% sample_clone_matrix[,(3*i-2)] == TRUE){ #The clone exist in the sample but has not been allocated yet. This only happens if they are equal in size.
                    ...
                    #subcloneswithinsample_order[rowtoremove,] <- "0" #210203. Testar att tysta 210514.
...
ML_treeplot <- function(ML_tree,limitml,col){
  if(col=="col"){
    ...
    EM_mltree <- EM_mltree +  theme_tree() + limitmp+
      scale_color_manual(values=c(certain_branches = "#FC4E07",uncertain_branches="darkgreen"))+
      theme(plot.title = element_text(hjust = 0.5, size = (14), color = "black"),legend.position = "none")
    print(EM_mltree)
  }else{
...
subclones <- function(EM_test,file_samples_subclones,root,possible_mothers,cutoff){
...
make_pie <- function(clonenames_new_order, root, samples, type, custom_col){
  ...
  # custom_col <- t(as.matrix(c(blue[1],blue[3],blue[4],blue[5], "M6
  #                             yellow[1],yellow[2],
  #                             red[1],red[3],
  #                             grey[6],grey[6])))
...
data <- load_matrix(filename="Segment.xlsx",sheetname ="Example_tumors") #Extracting the whole segment file including all of the tumors.
x <- "Tumor" #The tumor name.
...
#Add a fabricated ALL if you do not have a stem.
# fabricated_stem <- matrix(0,1,11)
# colnames(fabricated_stem) <- colnames(data)
# fabricated_stem[1,] <- c("WT21","ALL","1","1","1","NA","NA","Gain","TDS","Fabricated","100")
#datasegment <- rbind(fabricated_stem,datasegment)

#####################################
#Generating event matrices and trees#
#####################################
datatypes <- c("TDS","WES+TDS")#c(unique(test[,9])) #These are your data types such as SNP-array, TDS, WGS, WES etc. Change this vector if you do not want all of them to be included.
event_co <- 1000000
root <- "Normal"
...
EM_dev <- subclones(EM,file_samples_subclones,root = "Normal",possible_mothers,cutoff=30) #The first element in this list is the new event matrix. The second one is used for making pie charts.
DB <- distribution(overview_stem)
plot(DB)
ggsave(DB,filename= "hello.png",width = 10,height = 10)


#Visualizing the trees without pies and saving them
#EM_phy <- phydatevent(EM_dev[[1]][order,]) #Transforming the EM to phyDat format.
EM_phy <- phydatevent(EM_dev[[1]])
...
limitmp <- xlim(c(0, 2)) #Here you can determine the limits for the graph for mp.
limitml <- xlim(c(0,0.4)) #Here you can determine the limits for the graph for ml.
type <- "col"
...
coltype <- "nocol" #Choose how you want your pies. nocol = Just red pie charts with a biopsy name above. col = colored pies. custom = create your own color scheme.
...
pietree <- pie_it(Treeml,pieData,offset=0.2,size=0.21,col=coltype) #Adds pie charts to the tree. 0.21. Used 0.17 lately.
ggsave(pietree,filename=paste(x,"_solution1_MP",".png",sep=""),width = s,height = s) #RMS8_SNP_tree_ml