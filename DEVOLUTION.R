...
# rule <- matrix(0,1,2)
# rule[1,1] <- "9 p23-p23 Loss (1+0)"#"17p13q12 LOSS (1+0)"
# rule[1,2] <- "1 p34.2-q44 Gain (2+1)"#"17p13q12 GAIN (2+1)"
...
event_co <- 10000
root <- "Normal"
datasegment <- splitdata(data,name="Tumor1") #Extracting the beginning and end position of each sample in the segment file. #Declare which tumor you want to analyze. Specified by the first column in your data set.
...
starting_tree <- NJ(dm_h)
starting_tree <- root(starting_tree, outgroup = root,resolve.root = TRUE)
Lf <- pml(starting_tree, EM_phy) #Obtaining an object of class pml
Lf_JC <- optim.pml(Lf, model = "JC", optEdge = TRUE)
...
limitml <- xlim(c(0,1)) #Here you can determine the limits for the graph for ml.
type <- "nocol"
...
ggsave(Treemp,filename="Tree_mp.pdf",width=15,height=15)
...
p
ggsave(p,filename="Tree1_viridis.pdf",width=15,height=12)

#Add a heat map of the EM next to the tree.
q <- gheatmap(p,EM_dev[[1]], offset=1.5, width=5, 
...
q
ggsave(q,filename="Tree1_EM_ml.png",width=7,height=7)

#Add a heat map of the pies next to the tree.
df_pie<- tree_heatmap(clonenames_new_order)
q <- gheatmap(p,df_pie, offset=1, width=5, 
              colnames_angle=45, hjust=1,low="white",high="steelblue")+theme(legend.position="none")+
  scale_y_continuous(expand = expansion(mult = c(0.2,0)))
q
ggsave(q,filename="Tree1_pie_mp.png",width=7,height=7)
...
ggsave(pietree,filename=paste(x,"_col_mp",".png",sep=""),width = s,height = s) #RMS8_SNP_tree_ml
