...
data <- load_matrix(filename="PDX2.xlsx",sheetname ="Excluded_one")
x <- "Tumor"
...
DB <- distribution(overview) # If you want to plot an overview.
plot(DB)
...
limitmp <- xlim(c(0, 25)) #Here you can determine the limits for the graph for mp.
...
pietree <- pie_it(Treemp,pieData,offset=3,size=0.09,col="yes") #Adds pie charts to the tree. 0.17 is preferred. Lower to 0.09 for large phylogenies.
ggsave(pietree,filename=paste(x,"Treename",".pdf",sep=""),width = s,height = s)
